...
const productServiceImpl = {
  CreateProduct: async (call, callback) => {
    ...
    try {
      ...
      const productInsertResult = await client.query(
        `INSERT INTO public.productos (nombre, descripcion, imagen_data) VALUES ($1, $2, $3) RETURNING id`,
        [nombre, descripcion || null, imagen_data || null]
      );
  ...
  GetProducts: async (call, callback) => {
    try {
      const result = await pool.query(
        "SELECT id, nombre, descripcion, imagen_data FROM public.productos ORDER BY nombre ASC"
      );
  ...
  GetProduct: async (call, callback) => {
    ...
    if (!product_id) {
      return callback({
        code: grpc.status.INVALID_ARGUMENT,
        message: "Se requiere product_id para obtener el producto.",
      });
    }

    try {
      const result = await pool.query(
        `SELECT id, nombre, descripcion, imagen_data FROM public.productos WHERE id = $1`,
        [product_id]
      );
      ...
      callback(null, {
        ...
        imagen_data: productInfo.imagen_data || "",
      });
    } catch (error) {
      console.error("Error gRPC al obtener producto:", error);
      callback({
        code: grpc.status.INTERNAL,
